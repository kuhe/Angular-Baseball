...
class Ball extends AbstractMesh {
    ...
    /**
     * todo rotate in the correct direction based on the pitch type
     */
    rotate() {
    ...
    setRotation(rpm, clockDirection) {
        this.RPM = rpm;
    ...
    derivePitchingTrajectory(game) {
        ...
        var terminus = {
            x: (left - 100) * scale,
            y: (100 - top + 1.5 * ARC_APPROXIMATION_Y_ADDITIVE) * scale + Loop.VERTICAL_CORRECTION,
            z: 0
        };
        var breakingTerminus = {
            x: (breakLeft - 100) * scale,
            y: (100 - breakTop - 0.5 * ARC_APPROXIMATION_Y_ADDITIVE) * scale + Loop.VERTICAL_CORRECTION,
            z: 0
        };
