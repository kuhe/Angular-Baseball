...
Game.prototype = {
    ...
    passMinutes: function(minutes) {
        ...
        Animator.setLuminosity(1 - Math.abs(12 - this.timeOfDay.h)/12);
    },
    getInning : function() {
    ...
    theSwing : function(x, y, callback, override) {
        ...
        if (this.stage == 'swing') {
            ...
            if (this.stealAttempt === 'go' || this.stealAttempt === 'runnersDiscretion') {
                ...
                var valid = (!result.foul && !result.caught); // todo better define this
                var discretion = this.stealAttempt === 'runnersDiscretion' || Distribution.willSteal(pitch, this.pitcher.team.positions.catcher, thief, base);
                if (discretion && valid) {
                    thief.attemptSteal(this);
                }
    ...
    debugOut : function() {
        ...
        log('weak air hits (thrown out)', this.debug.filter(function(a){return !a.caught && !a.foul && a.flyAngle > 0 && a.thrownOut;}).length);
        log('good air hits (not caught)', this.debug.filter(function(a){return !a.caught && !a.foul && a.flyAngle > 0 && !a.thrownOut;}).length);
