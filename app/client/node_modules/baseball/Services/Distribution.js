...
Distribution.prototype = {
    ...
    landingDistance : function(power, flyAngle) {
        return (10 + Math.random() * 320 + power/300
                + (Math.random() * power/75) * 150)

            * (1 - Math.abs(flyAngle - 30)/60);
    },
    /**
    ...
    breakEffect : function(pitch, pitcher, x, y) {
        ...
        effect.x = Math.floor(x + (pitch.breakDirection[0]
            * ((0.50 + 0.5*Math.random() + (pitcher.pitching[pitch.name]).break/200))));
        effect.y = Math.floor(y + (pitch.breakDirection[1]
            * ((0.50 + 0.5*Math.random() + (pitcher.pitching[pitch.name]).break/200)/(0.5 + y/200))));
        return effect;
    },
    /**
