...
Field.prototype = {
    ...
    determineSwingContactResult: function determineSwingContactResult(swing) {
        ...
        if (swing.fielder) {
            ...
            var isOutfielder = fielder.position in { left: true, center: true, right: true };
            fielder.fatigue += 4;
            swing.error = false;
...
Game.prototype = {
    ...
    fromData: function fromData(data) {
        ...
        // const giraffe = this;
        var players = data.players || this.teams.away.lineup.concat(this.teams.home.lineup);
...
Log.prototype = {
    ...
    getSwing: function getSwing(swingResult) {
        ...
        var steal = '';
        var lineup = this.game.batter.team.lineup;
        if (!isNaN(swingResult.stoleABase)) {
            var thief = lineup[swingResult.stoleABase];
            steal = this.noteStealAttempt(thief, true, swingResult.attemptedBase);
        }
        if (!isNaN(swingResult.caughtStealing)) {
            var _thief = lineup[swingResult.caughtStealing];
            steal = this.noteStealAttempt(_thief, false, swingResult.attemptedBase);
        }
        if (steal) {
    ...
    getPlateAppearanceResult: function getPlateAppearanceResult(game) {
        ...
        if (r.looking) {
            ...
            if (!isNaN(r.stoleABase)) {
                var thief = lineup[r.stoleABase];
                steal = this.noteStealAttempt(thief, true, r.attemptedBase);
            }
            if (!isNaN(r.caughtStealing)) {
                var _thief2 = lineup[r.caughtStealing];
                steal = this.noteStealAttempt(_thief2, false, r.attemptedBase);
            }
            record += steal;
        } else {
            if (r.contact) {
