define(function(){
    ...
    Log.prototype = {
        ...
        noteSwing : function(pitchInFlight, swingResult) {
            ...
            this.pitchRecord.unshift(pitch.name + ' ('+Math.floor(pitch.x)+', '+Math.floor(pitch.y)+')');
            var swing = swingResult;
            this.pitchRecord.unshift('Swung at, ' + ' ('+Math.floor(swing.x)+', '+Math.floor(swing.y)+')' + (swing.contact ? ' and hit.' : ' and missed.'));
        },
        pointer : 0,
        pitchRecord : [],
        shortRecord : [],
